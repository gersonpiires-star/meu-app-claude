...
// estar válido) — ativo, dentro da validade, do limite de usos e (se
// restrito) usado pelo revendedor certo.
export async function validarCupom(
  codigoDigitado: string,
  revendedorId: string
): Promise<{ cupom: CupomValido } | { erro: string }> {
  ...
  if (cupom.revendedorId && cupom.revendedorId !== revendedorId) {
    return { erro: "Esse cupom não é válido para essa conta." };
  }
